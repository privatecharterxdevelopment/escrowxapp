...
var Id = new Uint8Array(new Array(16).fill(0).map((_3, i2) => i2));
var Pi = Id.map((i2) => (9 * i2 + 5) % 16);
var idxL = [Id];
var idxR = [Pi];
for (let i2 = 0; i2 < 4; i2++)
  for (let j2 of [idxL, idxR])
    j2.push(j2[i2].map((k3) => Rho[k3]));
var shifts = [
...
var shiftsL = idxL.map((idx, i2) => idx.map((j2) => shifts[i2][j2]));
var shiftsR = idxR.map((idx, i2) => idx.map((j2) => shifts[i2][j2]));
...
var y2 = 1;
var O2 = async (i2, t, e) => {
  const n2 = N, s2 = await t.getItem(n2);
  if (s2 && s2 >= y2) {
    e(t);
    return;
  }
  ...
  await t.setItem(n2, y2), e(t), j(i2, m2);
};
var j = async (i2, t) => {
...
//# sourceMappingURL=chunk-KLQIZRIO.js.map
