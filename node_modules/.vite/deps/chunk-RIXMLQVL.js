...
async function recoverPublicKey({ hash: hash2, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-VSOJOKHE.js");
  const signature_ = (() => {
...
async function prepareTransactionRequest(client, args_) {
  const attemptFill = (
    ...
    ["fees", "gas"].some((parameter) => {
      var _a;
      return (_a = args_.parameters) == null ? void 0 : _a.includes(parameter);
    })
  );
  const fillResult = attemptFill ? await getAction(client, fillTransaction, "fillTransaction")(args_).then((result) => {
    ...
    const error = e;
    if (error.cause.name === "MethodNotFoundRpcError" || error.cause.name === "MethodNotSupportedRpcError")
      supportsFillTransaction.set(client.uid, false);
    return args_;
  }) : args_;
  const { account: account_ = client.account, blobs, chain, gas, kzg, nonce, nonceManager: nonceManager2, parameters = defaultParameters, type } = fillResult;
...
//# sourceMappingURL=chunk-RIXMLQVL.js.map
